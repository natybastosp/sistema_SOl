// Sistema de Lógica Fuzzy para Recomendação Musical - Projeto SOL
// Arquivo consolidado compatível com a estrutura existente
...
class FuzzyMusicRecommendationEngine {
  ...
  async generateRecommendations(emotionalState, userId, playlistSize = 15) {
    ...
    const finalPlaylist = this.selectOptimalPlaylist(candidateMusics, emotionalState, playlistSize);
    const explanation = this.generateExplanation(ruleActivations, musicalStrategy);
  ...
  fuzzifyEmotionalState(emotionalState) {
    ...
    for (const rule of this.rules) {
      ...
      if (activation > 0.1) {
        console.log(`   ✓ Regra "${rule.description}": ${(activation * 100).toFixed(1)}% ativada`);
      }
    }
  ...
  inferMusicalStrategy(ruleActivations) {
    ...
    for (const rule of this.rules) {
      ...
      if (activation > 0.05) {
        ...
        const valenceAvg = (rule.musicalRecommendations.valenceRange[0] + 
                          rule.musicalRecommendations.valenceRange[1]) / 2;
        const energyAvg = (rule.musicalRecommendations.energyRange[0] + 
                          rule.musicalRecommendations.energyRange[1]) / 2;
        const danceabilityAvg = (rule.musicalRecommendations.danceabilityRange[0] + 
                                rule.musicalRecommendations.danceabilityRange[1]) / 2;
  ...
  async findCandidateMusics(strategy) {
    ...
    try {
      ...
      console.warn('⚠️ Erro na busca específica, usando busca geral:', error.message);
      ...
      console.log(`   ✓ Usando ${fallbackCandidates.length} músicas como fallback`);
  ...
  selectOptimalPlaylist(candidates, originalEmotionalState, playlistSize) {
    ...
    console.log(`   ✓ Playlist final com ${selectedMusics.length} músicas selecionada`);
    return selectedMusics;
  }

  calculateMusicFuzzyScore(music, emotionalState) {
    const emotionalProximity = this.calculateEmotionalProximity(music, emotionalState);
    const therapeuticPotential = this.calculateTherapeuticPotential(music, emotionalState);
    const musicalQuality = 0.5; // Score base quando não temos dados de popularidade
    
    return (emotionalProximity * 0.4) + 
  ...
  calculateTherapeuticPotential(music, emotionalState) {
    ...
    if (emotionalState.sadness > 0.7) {
      ...
      if (valence > emotionalState.sadness && valence < emotionalState.sadness + 0.3) {
  ...
  generateExplanation(ruleActivations, strategy) {
    ...
    if (activeRules.length === 0) {
      return "Baseei as recomendações em seu estado emocional equilibrado, " +
             "selecionando músicas que mantenham estabilidade emocional.";
    }

    const primaryRule = this.rules.find(rule => rule.id === activeRules[0][0]);
    const explanation = `Detectei que ${primaryRule?.description.toLowerCase()}. ` +
    ...
    return explanation + `combinam com este estado, priorizando gêneros como ` +
  ...
  calculateConfidence(ruleActivations) {
    ...
    return Math.min(1.0, Math.max(maxActivation, totalActivation / this.rules.length));
  }
}
...
async function generateFuzzyMusicRecommendation(request) {
  ...
  try {
    ...
    console.log(`✅ Recomendação gerada com ${recommendation.playlist.length} músicas`);
    console.log(`🎯 Confiança: ${(recommendation.confidence * 100).toFixed(1)}%`);
