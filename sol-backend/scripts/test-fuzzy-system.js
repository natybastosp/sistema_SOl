...
const fuzzyModulePath = path.join(__dirname, '..', 'src', 'lib', 'fuzzy', 'emotionalMusicRecommendation.js');
...
const testCases = {
  balanced: {
    ...
    emotionalState: { anger: 0.1, fear: 0.2, joy: 0.6, sadness: 0.1, surprise: 0.3 }
  },
  very_sad: {
    name: 'Tristeza Profunda', 
    description: 'Pessoa passando por momento de tristeza intensa',
    emotionalState: { anger: 0.2, fear: 0.4, joy: 0.1, sadness: 0.8, surprise: 0.1 }
  },
  anxious: {
    name: 'Ansiedade Alta',
    description: 'Pessoa com altos níveis de ansiedade e apreensão', 
    emotionalState: { anger: 0.3, fear: 0.7, joy: 0.2, sadness: 0.4, surprise: 0.5 }
  },
  very_happy: {
    name: 'Alegria Intensa',
    description: 'Pessoa em estado de alegria e felicidade elevadas',
    emotionalState: { anger: 0.1, fear: 0.1, joy: 0.9, sadness: 0.0, surprise: 0.6 }
  }
};
...
async function runFuzzyTests() {
  ...
  for (const [testId, testCase] of Object.entries(testCases)) {
    ...
    try {
      ...
      console.log(`   🎯 Confiança: ${(recommendation.data.confidence * 100).toFixed(1)}%`);
      console.log('');
      ...
      if (recommendation.data.playlist && recommendation.data.playlist.length > 0) {
        ...
        recommendation.data.playlist.slice(0, 3).forEach((music, index) => {
          ...
          console.log(`      Gênero: ${music.genre} | Score: ${(music.fuzzyScore * 100).toFixed(1)}%`);
        });
        
        if (recommendation.data.playlist.length > 3) {
          console.log(`   ... e mais ${recommendation.data.playlist.length - 3} músicas`);
        }
      } else {
