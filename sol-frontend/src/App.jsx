...
import { Play, Pause, SkipForward, ThumbsUp, ThumbsDown, User, Music, Brain, Sun } from 'lucide-react';
...
const SolMusicSystem = () => {
  ...
  const genres = ['Rock', 'Pop', 'MPB', 'Sertanejo', 'Funk', 'Jazz', 'Clássica', 'Eletrônica', 'Reggae', 'Hip Hop'];
  ...
  const SunLogo = ({ size = 'large' }) => {
    ...
    return (
      <div className={`${sizeClasses[size]} bg-gradient-to-br from-orange-300 to-orange-400 rounded-full mx-auto mb-6 flex items-center justify-center shadow-lg`}>
  ...
  const handleTrackFeedback = (trackId, rating) => {
              ...
              {currentPlaylist.length > 0 && (
                <>
                  <div className="text-center mb-6">
                    ...
                    <p className="text-gray-600">{currentPlaylist[currentTrack]?.artist}</p>
                    <p className="text-sm text-gray-500">{currentPlaylist[currentTrack]?.duration}</p>
                  </div>
                  ...
                  <div className="flex justify-center space-x-4">
                    <Button
                      onClick={() => handleTrackFeedback(currentPlaylist[currentTrack].id, 'positive')}
                      variant={feedback[currentPlaylist[currentTrack].id] === 'positive' ? 'default' : 'outline'}
                    ...
                    <Button
                      onClick={() => handleTrackFeedback(currentPlaylist[currentTrack].id, 'negative')}
                      variant={feedback[currentPlaylist[currentTrack].id] === 'negative' ? 'default' : 'outline'}
                      className="flex items-center space-x-2"
                    >
      ...
      <div className="flex-1 p-8">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
            <div className="bg-white rounded-lg shadow-sm p-6">
              <h3 className="text-lg font-semibold text-gray-800 mb-2">Sessões Totais</h3>
              <p className="text-3xl font-bold text-orange-500">{emotionalHistory.length}</p>
            </div>
            
            <div className="bg-white rounded-lg shadow-sm p-6">
              <h3 className="text-lg font-semibold text-gray-800 mb-2">Músicas Ouvidas</h3>
              <p className="text-3xl font-bold text-orange-500">
                {emotionalHistory.reduce((acc, entry) => acc + entry.tracksPlayed, 0)}
              </p>
            </div>
            
            <div className="bg-white rounded-lg shadow-sm p-6">
              <h3 className="text-lg font-semibold text-gray-800 mb-2">Satisfação Média</h3>
              <p className="text-3xl font-bold text-orange-500">
                {emotionalHistory.length > 0 
                  ? Math.round((emotionalHistory.reduce((acc, entry) => acc + entry.satisfaction, 0) / emotionalHistory.length) * 100) / 100
                  : 0}
              </p>
            </div>
          </div>
